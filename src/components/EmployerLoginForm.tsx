...
import { employerLoginSuccess } from '~/features/auth/employerAuthSlice'
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (response?.status === 200) {
                dispatch(employerLoginSuccess({ employerToken: response.data.token, company: response.data.company }))
    ...
    return (
        <form className='px-16 py-12 flex flex-col gap-6 bg-white rounded-lg' onSubmit={onSubmit}>
            <div className='mx-auto '>
                <h1 className='font-serif text-4xl font-bold text-cyan-600'>IT Talent</h1>
