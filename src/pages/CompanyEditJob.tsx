...
const CompanyEditJob = () => {
    ...
    const processResponseData = (data: {
        ...
        const result: FormState = {
           ...
           phases: rest.phases.join(', ')
        }
        return result
    }

    const processFormData = (data: FormState) => {
        const { categories, requiredSkills,phases, ...rest } = data
        return {
            ...rest,
            categories: categories.split(',').map((item) => item.trim()),
            requiredSkills: requiredSkills.split(',').map((item) => item.trim()),
            phases: phases.split(',').map((item) => item.trim())
        }
    }
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!params.id) return
        const res = await updateJob(params.id, processFormData(form),token)
        if (res?.status === 200) {
